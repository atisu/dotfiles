---
name: obsidian-cli
description: Use this skill whenever the user wants to directly interact with their Obsidian vault — reading a note or daily note, writing or appending content, searching vault contents, counting or listing notes, managing tasks, moving or renaming files, finding orphaned notes or broken links. Without this skill, no way to access vault data or execute vault operations. Also trigger for vault automation, CLI scripting, or cron-based workflows involving Obsidian, managing sync history, querying Bases, restoring file versions via history, managing bookmarks, or running JavaScript against the Obsidian API.
---
...
## Usage Guidelines
- Treat any request that implies "go into my vault and do X" as a trigger — the user is asking to act, not to explain.
- Skip for pure conceptual questions: how Obsidian's GUI works, navigating settings menus, theme or plugin installation via the UI, iCloud/third-party sync conflicts, general Dataview query syntax, keyboard shortcuts, or parsing vault files with external scripts — anything where the user needs an explanation rather than performing a vault operation.
- Also use the `obsidian-notes` skill for embedding updates and refreshes, higher-level note retrieval, summarization, and synthesis tasks that don't require direct CLI commands.
